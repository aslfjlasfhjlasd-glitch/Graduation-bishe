...
const fetchData = async () => {
  ...
  errorMessage.value = ''
  try {
    const res = await axios.get(`${API_BASE}/api/student/leave-permits`, { params: { studentId } })
    if (res.data.code === 200) {
      records.value = res.data.data || []
    } else {
      errorMessage.value = res.data.message || '获取公假单失败'
    }
  } catch (e) {
    console.error(e)
...
const downloadPdf = async (record) => {
  const studentId = localStorage.getItem('studentId')
  try {
    const res = await axios.get(`${API_BASE}/api/student/leave-permits/${record.id}/download`, {
      params: { studentId },
      responseType: 'blob'
    })
    ...
    a.href = url
    a.download = `leave-permit-${record.id}.pdf`
    a.click()
...
<template>
  <div class="space-y-6">
    <h2 class="text-2xl font-bold text-slate-900">公假单下载</h2>
    ...
    <Card>
      ...
      <CardContent>
        ...
        <div v-else-if="records.length === 0" class="text-center py-12 text-slate-400">
          ...
          <p class="text-lg mb-2">暂无已确认公假单</p>
          <p class="text-sm">仅展示已出具的公假单</p>
        </div>
        <div v-else class="space-y-4">
          ...
          >
            <div class="space-y-1">
              ...
              <p class="text-sm text-slate-600">时间：{{ item.activityTime || '-' }}</p>
              <p class="text-xs text-slate-500">出具时间：{{ formatDateTime(item.leaveIssuedAt) }}</p>
            </div>
