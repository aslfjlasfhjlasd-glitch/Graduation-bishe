...
public interface DashboardMapper {
    ...
    @Select("SELECT HD_BH AS id, HD_MC AS hdmc, " +
            ...
            "HD_DD AS hddd, ZM_RS AS zyrs, YBM_RS AS ybmrs FROM t_zyhd ORDER BY HD_BH DESC")
    ...
    @Select("SELECT HD_BH AS id, HD_MC AS hdmc, " +
            ...
            "DATE_FORMAT(HD_KSSJ, '%Y-%m-%d %H:%i:%s') AS hdkssj, " +
            "DATE_FORMAT(HD_JSSJ, '%Y-%m-%d %H:%i:%s') AS hdjssj, " +
            "HD_NR AS hdnr, HD_DD AS hddd, ZM_RS AS zyrs, YBM_RS AS ybmrs, HDXQ AS hdxq, HD_BZ AS hdbz, HD_FQ_DW AS hdfqdw, HD_ZT AS hdzt FROM t_zyhd WHERE HD_BH = #{id}")
